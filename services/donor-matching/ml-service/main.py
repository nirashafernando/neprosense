import sys
# Force UTF-8 I/O when stdout/stderr is a pipe (e.g. under concurrently on Windows)
...
def load_model():
    ...
    try:
        ...
        logger.info(f"[OK] Model loaded successfully from: {model_path.name}")
        ...
        if SHAP_AVAILABLE and model is not None:
            try:
                ...
                if hasattr(model, 'named_steps') and 'classifier' in model.named_steps:
                    ...
                    logger.info("[OK] SHAP explainer initialized for Random Forest classifier")
                else:
                    shap_explainer = shap.TreeExplainer(model)
                    logger.info("[OK] SHAP explainer initialized")
...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        if hla_match_score == 6 and blood_compatible:
            ...
            logger.info(f"[BOOST] HLA 6/6 PERFECT MATCH BOOST: {original_prob:.4f} -> {probability:.4f} (+28%)")
...
async def predict_batch(request: BatchPredictionRequest):
    ...
    try:
        ...
        for donor in donors:
            ...
            if model is not None:
                try:
                    ...
                    logger.debug(f"Donor {donor_id}: raw_proba={probability:.4f}")
                    ...
                    if hla_match_score == 6 and blood_compatible:
                        original_prob = probability
                        probability = min(probability + 0.28, 0.98)  # Boost by 28%, cap at 98%
                        logger.debug(f"HLA 6/6 BOOST: {original_prob:.4f} -> {probability:.4f}")
            ...
            logger.debug(f"Storing: {donor_id} probability={stored_probability} risk={risk_category['category']}")
        ...
        logger.info(f"[OK] Batch prediction completed: {len(predictions)} donors evaluated")
