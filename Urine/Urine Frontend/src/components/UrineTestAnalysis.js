    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6 font-sans">
      <div className="w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl p-10 border border-slate-100">
        <div className="text-center mb-10">
          {/* (green-50, green-600) */}
          <div className="inline-flex items-center justify-center p-4 bg-green-50 rounded-2xl mb-4"><User className="text-green-600 w-8 h-8" /></div>
        ...
        <div className="space-y-6">
          <div>
            ...
            <input type="text" className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none focus:bg-white focus:border-green-500 transition-all font-bold text-slate-700" placeholder="e.g. Ashini Perera" onChange={(e) => setFormData({...formData, patientName: e.target.value})} />
          </div>
          <div className="grid grid-cols-2 gap-6">
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Age</label>
              <input type="number" className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none focus:bg-white focus:border-green-500 transition-all font-bold text-slate-700" placeholder="Years" onChange={(e) => setFormData({...formData, age: e.target.value})} />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Gender</label>
              <select className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none focus:bg-white focus:border-green-500 transition-all font-bold text-slate-700" onChange={(e) => setFormData({...formData, gender: e.target.value})}>
          ...
          <div>
            ...
            <input type="date" value={formData.testDate} className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none focus:bg-white focus:border-green-500 transition-all font-bold text-slate-700" onChange={(e) => setFormData({...formData, testDate: e.target.value})} />
          </div>
          {/*  (bg-green-600, hover:bg-green-700) */}
          <button onClick={handleSubmit} className="w-full py-5 bg-green-600 text-white font-black rounded-2xl shadow-xl hover:bg-green-700 transition-all transform active:scale-95 flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen p-8 bg-slate-50 flex items-center justify-center font-sans">
      <div className="w-full max-w-3xl">
        <button onClick={onBack} className="flex items-center text-slate-400 hover:text-green-600 font-bold mb-8 transition-colors"><ArrowLeft size={18} className="mr-2" /> GO BACK</button>
        <div className="bg-white rounded-[3rem] p-12 shadow-2xl border border-slate-100 text-center">
          ...
          {!preview ? (
            <label className="block border-4 border-dashed border-slate-100 rounded-[2rem] p-20 hover:border-green-200 cursor-pointer group transition-all">
              <input type="file" className="hidden" onChange={handleUpload} accept="image/*" />
              <div className="flex flex-col items-center">
                <div className="w-20 h-20 bg-green-50 rounded-3xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform"><Camera className="w-10 h-10 text-green-600" /></div>
                <span className="text-xl font-bold text-slate-400 group-hover:text-green-600">Click to upload photo</span>
              </div>
            </label>
          ) : (
            <div className="space-y-8">
              <img src={preview} className="rounded-[2rem] shadow-2xl border-4 border-white mx-auto max-w-sm" alt="Preview" />
              <button onClick={() => onNext({ image: preview, imageFile: file })} className="px-12 py-5 bg-green-600 text-white font-black rounded-2xl shadow-xl hover:bg-green-700 transition-all flex items-center gap-3 mx-auto">
