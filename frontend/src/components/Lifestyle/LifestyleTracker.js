...
const LifestyleTracker = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:8080/api/lifestyle/save-entry", {
        method: "POST",
        headers: {
      ...
      alert("Error connecting to server! Check if the API Gateway (http://localhost:8080) is running.");
    } finally {
      setLoading(false);
    }
  };
