      ...
      <div className="w-full max-w-md p-8 bg-white border shadow-2xl rounded-3xl border-slate-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            ...
            <input type="text" className="w-full p-3 border outline-none bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-medical-700" onChange={(e) => setFormData({ ...formData, patientName: e.target.value })} required />
          </div>
          <div className="space-y-1">
            <label className="ml-1 text-xs font-bold uppercase text-slate-500">Patient ID</label>
            <input type="text" className="w-full p-3 border outline-none bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-medical-700" onChange={(e) => setFormData({ ...formData, patientId: e.target.value })} required />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="ml-1 text-xs font-bold uppercase text-slate-500">Age</label>
              <input type="number" className="w-full p-3 border outline-none bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-medical-700" onChange={(e) => setFormData({ ...formData, age: e.target.value })} required />
            </div>
            <div className="space-y-1">
              <label className="ml-1 text-xs font-bold uppercase text-slate-500">Gender</label>
              <select className="w-full p-3 border outline-none bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-medical-700" value={formData.gender} onChange={(e) => setFormData({ ...formData, gender: e.target.value })}>
          ...
          <div className="space-y-1">
            ...
            <input type="date" value={formData.date} className="w-full p-3 border outline-none bg-slate-50 border-slate-200 rounded-xl focus:ring-2 focus:ring-medical-700" onChange={(e) => setFormData({ ...formData, date: e.target.value })} required />
          </div>
          <button type="submit" className="flex items-center justify-center w-full gap-2 py-4 mt-4 font-bold text-white transition-all bg-medical-700 shadow-lg rounded-xl hover:bg-medical-800">
  ...
  return (
    <div className="flex items-center justify-center min-h-[calc(100vh-80px)] p-6 bg-slate-100">
      <div className="w-full max-w-xl p-10 text-center bg-white border shadow-2xl rounded-3xl border-slate-200">
        <button onClick={onBack} className="flex items-center mb-6 font-medium transition-all text-slate-500 hover:text-medical-700">
        ...
        <div className="relative p-12 transition-all border-2 border-dashed cursor-pointer border-slate-300 rounded-3xl hover:border-medical-700 bg-slate-50 group">
          ...
          <label htmlFor="file" className="cursor-pointer">
            {previewUrl ? <img src={previewUrl} className="mx-auto border-4 border-white shadow-md max-h-64 rounded-xl" alt="Preview" /> : <div className="flex flex-col items-center gap-4 py-10">
              <Upload size={48} className="transition-colors text-slate-300 group-hover:text-medical-600" />
              <span className="font-bold text-slate-400">Select Image to Upload</span>
            </div>}
          </label>
        </div>
        {previewUrl && (
          <button onClick={() => onNext({ file: file })} className="w-full py-4 mt-8 font-bold text-white transition-all bg-medical-700 shadow-xl rounded-xl hover:bg-medical-800">
    ...
    <div className="flex flex-col items-center min-h-screen p-8 bg-slate-200">
      <div ref={reportRef} className="w-[210mm] min-h-[297mm] p-16 bg-white shadow-2xl font-sans text-slate-900 flex flex-col">
        ...
        <div className="flex items-start justify-between pb-8 mb-10 border-b-4 border-medical-700">
          <div>
            <h1 className="text-4xl font-black tracking-tighter text-medical-900">KIDNEY SCAN AI</h1>
        ...
        <div className="grid grid-cols-2 p-6 mb-12 border gap-x-12 gap-y-3 bg-slate-50 rounded-2xl border-slate-100">
          ...
          <p className="text-sm"><b>AGE:</b> <span className="ml-2 text-slate-600">{testData.age} Years</span></p>
          <p className="text-sm"><b>GENDER:</b> <span className="ml-2 text-slate-600">{testData.gender}</span></p>
        ...
        <div className="p-8 mb-12 border border-medical-100 bg-medical-50/50 rounded-3xl">
          <h3 className="mb-3 text-xs font-black text-medical-900 uppercase">III. CLINICAL IMPRESSION</h3>
      ...
      <div className="flex gap-4 pb-20 mt-10">
        <button onClick={downloadPDF} className="flex items-center gap-2 px-10 py-5 text-sm font-bold text-white uppercase transition-all bg-medical-700 shadow-2xl rounded-2xl hover:bg-medical-800">
...
const KidneyUltrasoundSystem = () => {
  ...
  const handlePredict = async (uploadData) => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5000/api/predict', { method: 'POST', body: formDataPayload });
